...
#With help of the function glimpse we list down the data file
glimpse(weather_1)
...
tidyweather1 <- tidyweather %>%
  ...
  #With the function mutate we are changing the characteristics of the data and creating new variables
...
ggplot(tidyweather1, aes(x = date, y = delta, color = "#FF6347")) +
  ...
  #Legend does not add value to the plot, therefore we exclude it
  theme(legend.position = "none") +
  
  #Add title and names and source to the axes,
...
ggplot(tidyweather1, aes(x = date, y = delta, colour = "#FF6347"))+
  ...
  #Name axes and add title and source
  labs(
...
#We create new variable
average_annual_anomaly <- tidyweather %>% 
  
  #we group the data by years
  group_by(Year) %>%
  
  #we calculate the averages of year increase. Exclude NAs from calculation
  summarise(annual_average_delta = mean(delta, na.rm=TRUE)) 

#We have everything prepared, let's plot a chart
ggplot(average_annual_anomaly, aes(x=Year, y= annual_average_delta, colour = "#FF6347"))+
  ...
  #Legend does not add value, therefore we exclude it
  theme(legend.position = "none")  +
...
formula_ci <- comparison %>% 
  ...
  #Filter for interval year 2011 and onwards
  filter(interval == "2011-present") %>% 
  
  #With summarise function we construct CI
  summarise(
...
#Calculate confidence interval using bootstrap function
comparison <- tidyweather %>% 
...
infer_confidence_interval <- bootstrap_delta %>% 
