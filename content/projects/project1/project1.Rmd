...
1. We selected the year and the twelve month variables from the `weather` dataset. We do not need the others (J-D, D-N, DJF, etc.) for this assignment. 
...
2. We convert the dataframe from wide to 'long' format. We named the new dataframe as `tidyweather`, name the variable containing the name of the month as `month`, and the temperature deviation values as `delta`.
...
#plot the graph
ggplot(tidyweather, aes(x=date, y = delta, color = "#FF6347")) +
  geom_point() +
  geom_smooth(color = "dark blue")+
  theme_bw() + 
  theme(legend.position = "none") +
  labs(title= "Annual temperature deviation relative to 1951-1980 base",
...
Is the effect of increasing temperature more pronounced in some months? We used `facet_wrap()` to produce a seperate scatter plot for each month, again with a smoothing line.

```{r facet_wrap, echo=FALSE}

ggplot(tidyweather, aes(x=date, y = delta, colour = "#FF6347"))+
  geom_point()+
...
We removed data before 1800 and before using `filter`. Then, we used the `mutate` function to create a new variable `interval` which contains information on which period each observation belongs to. We assigned the different periods using `case_when()`.
